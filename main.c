...
void *addCounter(void *arg) {
    ...
    while (1) {
        ...
        sem_wait(&counter_sem);//ensure that only one thread at a time can execute the critical section code
        //Critical Section start/
        counter++;//Critical Section increasing counter
        //Critical Section end/
        printf("Counter thread %d: now adding to counter,counter value=%d\n\n", threadNum, counter);//printing new counter value
...
//
        ...
        if (temp == SIZE) {//checking if the value is full==SIZE then this means that full was incremented till it reached the size and buffer is full
            printf("Monitor thread: Buffer full! \n\n");
        }
...
void *collect(void *arg) {
    while (1) {
        ...
        if (temp == SIZE) {//if empty == SIZE it means that we didn't put anything in the buffer bec empty_sem is initialized with SIZE
...
int main() {
    ...
    sem_init(&counter_sem, 0, 1);//initializing counter_sem with 1 which means that only one thread can access critical section at a time
    sem_init(&empty_sem, 0, SIZE);//initializing empty_sem with SIZE which means that we can insert up to SIZE items in the buffer
    sem_init(&full_sem, 0, 0);//Initialzing full_sem with zero which means that we can take up to SIZE items from the buffer
    ...
    int n;
    printf("enter number of count threads:");
    scanf("%d",&n);
    //int n = THREADSNUM;
